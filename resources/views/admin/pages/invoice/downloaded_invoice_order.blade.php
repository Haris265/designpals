    ...
    <div class="container">
        <div class="logobar">
            <img src="{{ public_path('assets/img/Logo.png') }}" style="margin-bottom:10px" alt="logo" height="80"
                width="200" />
            <h3 style="margin-top: 20px">INVOICE NO: {{$namePrefix}}</h3>
        ...
        <div class="table">
            <table>
                ...
                <tbody>
                    ...
                    $i = 0;
                   @endphp
                   @if(!empty($invoices->downloadedOrder))
                    @foreach ($invoices->downloadedOrder as $order)
                        <tr>
                            <td>
                                @if (!empty($order))
                                @php $symbol = $order->order->currency_symbol;
                                $i = $i+1;
                                @endphp
                                    {{$loop->iteration}}
                                @endif
                            ...
                            <td>
                                @if (!empty($order))
                                    {{ \Carbon\Carbon::parse($order->order->date)->format('j-M-y') }}
                                @endif
                            ...
                            <td>
                                @if (!empty($order))
                                    {{ $order->order->currency_symbol . $order->order->price }}
                                @endif
                            </td>
                        </tr>
                    @endforeach
