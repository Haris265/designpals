    ...
    <style>
        * {
        ...
        .mybtn {
            ...
            font-weight: bolder;
            box-shadow: 2px 2px 2px 2px border:2px solid #bd9c06;
        }
        ...
        <div class="table">
            <table>
                ...
                {{-- <tbody>
                    ...
                    <tr>
                        ...
                        <td style="font-weight: bold">
                            @if ($invoice->invoice_type == 'us')
                                {{ $symbol . $client->total_order_price }}
                            @elseif($invoice->invoice_type == 'uk')
                                {{ $symbol . $client->total_order_price }}
                            @endif
                        </td>
                ...
                <tbody>
                    ...
                    $i = 1;  
                    $symbol = "$";
                    @endphp {{-- Counter initialized outside the loop --}}
                    @if (!empty($client->order))
                        @foreach ($client->order as $order)
                            <tr>
                                <td>{{ $i }}</td> {{-- Displaying the correct serial number --}}
                                <td>{{ !empty($order) ? \Carbon\Carbon::parse($order->date)->format('j-M-y') : '' }}</td>
                                <td>{{ !empty($order) ? $order->order_name : '' }}</td>
                                <td>{{ !empty($order) ? $order->currency_symbol . $order->price : '' }}</td>
                            </tr>
                            @php $i++; @endphp {{-- Incrementing after each row --}}
                        @endforeach
                    @endif
                    ...
                    <tr>
                        ...
                        <td style="font-weight: bold">
                            @if ($invoice->invoice_type == 'us')
                                {{ $order->currency_symbol . $client->total_order_price }}
                            @elseif($invoice->invoice_type == 'uk')
                                {{ $order->currency_symbol . $client->total_order_price }}
                            @endif
                        </td>
